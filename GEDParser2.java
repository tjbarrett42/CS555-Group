...
public class GEDParser {   
    ...
    /* Somya Gambhir */
    private static DatesAfterCurrent DatesAfterCurrent;
    private static us24 us24; //FUNCTIONAL
    private static RejectIllDates RejectIllDates; //FUNCTIONAL
    private static BirthBeforeDeath us03; //FUNCTIONAL
    private static ListDeceased ListDeceased; //FUNCTIONAL
    private static ListLivingSingle ListLivingSingle; //FUNCTIONAL
    private static MaleLastNames MaleLastNames; //FUNCTIONAL
    private static BirthBeforeMarriage BirthBeforeMarriage; //FUNCTIONAL
    ...
    public static void checkIndividuals() {
        ...
        for (int i = 0; i < prntind.length; i++) {
           ...
           // UniqueIDs.checkUniqueIDs(individuals[i]); // No print statement needed, handles them itself
            ...
            if(!us03.CheckUS03(individuals[i])){
                System.out.println("ERROR: INDIVIDUAL: US03: " + prntind[i][0] + " cannot be born after death!");
            }
    ...
    public static void checkFamilies() {
        String[][] prntfam = new String[famcounter][8];
        ...
        for (int i = 0; i < prntfam.length; i++) {
         //   if (!MarriageBeforeDeath.checkMarriageBeforeDeath(families[i])) {
           //     System.out.println("ERROR: Family " + families[i][0] + " were married after the death of a spouse!");
            //}
            BirthBeforeMarriage.CheckUS02(prntfam[i],prntind);
            if (!MarriageBeforeDivorce.checkMarriageBeforeDivorce(prntfam[i])) {
                System.out.println("ERROR: Family " + prntfam[i][0] + " were divorced before they were married!");
            }
            if (!DivorceBeforeDeath.checkDivorceBeforeDeath(prntfam[i])) {
                System.out.println("ERROR: Family " + prntfam[i][0] + " were divorced after the death of a spouse!");
            }
        }
        us24.us24(prntfam);
    }
    ...
    public static void noLoop(){
        ...
        DatesAfterCurrent.DatesAfterCurrent(prntind,prntfam);
        MaleLastNames.CheckMaleLastNames(prntind,prntfam);
        RejectIllDates.RejectIllDates(prntind,prntfam);
        ListDeceased.CheckListDeceased(prntind);
        ListLivingSingle.ListLivingSingle(prntind);
    }
